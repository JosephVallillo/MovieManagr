...
extension TMDBClient {
    ...
    */
    ...
    private func getSessionID(requestToken: String?, completionHandlerForSession: (success: Bool, sessionID: String?, errorString: String?) -> Void) {
        ...
        parameters = [TMDBClient.ParameterKeys.RequestToken : requestToken!]
    ...
    private func getUserID(completionHandlerForUserID: (success: Bool, userID: Int?, errorString: String?) -> Void) {
        ...
        /* 1. Specify parameters, the API method, and the HTTP body (if POST) */
        var parameters = [String : AnyObject]()
        parameters = [TMDBClient.ParameterKeys.SessionID : self.sessionID!]
        ...
        taskForGETMethod(Methods.Account, parameters: parameters) { (result, error) -> Void in
            ...
            if let error = error {
                print(error)
                completionHandlerForUserID(success: false, userID: nil, errorString: "Login Failed (Used ID")
            } else {
        ...
        taskForGETMethod(mutableMethod, parameters: parameters) { JSONResult, error in
            ...
            if let error = error {
                ...
                if let results = JSONResult[TMDBClient.JSONResponseKeys.MovieResults] as? [[String : AnyObject]] {
                    ...
                    completionHandlerForWatchlist(result: nil, error: NSError(domain: "getFavoriteMovies parsing", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not parse getFavoriteMovies"]))
                }
            }
        }
    ...
    func getMoviesForSearchString(searchString: String, completionHandlerForMovies: (result: [TMDBMovie]?, error: NSError?) -> Void) -> NSURLSessionDataTask? {
        ...
        /* 2. Make the request */
        let task = taskForGETMethod(Methods.SearchMovie, parameters: parameters) { (JSONresult, error) -> Void in
    ...
    func postToFavorites(movie: TMDBMovie, favorite: Bool, completionHandlerForFavorite: (result: Int?, error: NSError?) -> Void)  {
        ...
        mutableMethod = client.subtituteKeyInMethod(mutableMethod, key: TMDBClient.URLKeys.UserID, value: String(TMDBClient.sharedInstance().userID!))!
        ...
        taskForPOSTMethod(mutableMethod, parameters: parameters, jsonBody: jsonBody) { JSONResult, error in
            ...
            if let error = error {
                ...
                if let results = JSONResult[TMDBClient.JSONResponseKeys.StatusCode] as? Int {
                    ...
                    completionHandlerForWatchlist(result: nil, error: NSError(domain: "postToFavoritesList parsing", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not parse postToFavoritesList"]))
                }
            }
        }
